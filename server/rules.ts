...
export default class Rules {
    ...
    constructor(config: Config, providers: Providers) {
        ...
        providers.on("register", (service: Service) => {
            this.#rules.filter(r => r.watchServices.includes(service.uniqueId)).forEach(r => r.run())
        })

        providers.on("update", (service: Service, key: string) => {
            this.#rules.filter(r => r.watchProperties.includes(`${service.uniqueId}/${key}`)).forEach(r => r.run())
        })

        config.forEach((r) => this.#rules.push(new JSRule(r, this)))
        setImmediate(() => this.#rules.forEach(r => r.run()))
    }
}
